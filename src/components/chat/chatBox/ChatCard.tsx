...
const ChatCard = ({title, subtitle, url}: Recommendation) => {
    return (
        <a href={url}>
            <div className="drop-shadow-md rounded-lg w-32 lg:w-64 min-h-full flex flex-col bg-white">
                ...
                <div className="grow pt-2 px-3 lg:px-4 pb-3 bg-sky-500 rounded-b-lg text-white flex flex-col items-center">
                  ...
                  { subtitle ? <div className="w-full p-1 text-slate-200 break-words">{subtitle}</div> : "" }
                </div>
            </div>
        </a>
    )
